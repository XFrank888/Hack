chrome.storage.sync.get(null, result => {
    ...
    if (!result.assignments && !result.grades){
        ...
        for (const [key, value] of Object.entries(result.assignments)){
            ...
            for (var i = 0; i < value.length; i++){
                ...
                assignmentsDiv.innerHTML += `<p>&emsp;&emsp;&emsp; due ${value[i].dueDate +" " + value[i].time}</p>`;
    ...
    var seconds = Math.floor(((new Date()).getTime() - result.lastUpdateTime) / 1000);
    ...
    if (days > 0) {
        if (days == 1)
            s.innerHTML += `<p> Last update: ${days} day ago</p>`;
        else
            s.innerHTML += `<p> Last update: ${days} days ago</p>`;
    }
    else if (hours > 0) {
        if (hours == 1)
            s.innerHTML += `<p> Last update: ${hours} hour ago</p>`;
        else
            s.innerHTML += `<p> Last update: ${hours} hours ago</p>`;
    }
    else if (minutes > 0) {
        if (minutes == 1)
            s.innerHTML += `<p> Last update: ${minutes} minute ago</p>`;
        else
            s.innerHTML += `<p> Last update: ${minutes} minutes ago</p>`;
    }
    else {
        if (seconds == 1)
            s.innerHTML += `<p> Last update: ${seconds} second ago</p>`;
        else
            s.innerHTML += `<p> Last update: ${seconds} seconds ago</p>`;
    }
